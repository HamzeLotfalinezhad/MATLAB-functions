function rqa_stat=RQA(x,dimention,delay,treshold,LineMinLength)
% rqa_stat=[DET LAM LMAX VMAX L TT ENT RATIO  RR DIV];
if nargin <2
    fprintf('(x,dimention,delay,treshold,LineMinLength)');
end
Y=psr_deneme(x,dimention,delay)';
%
DM=dist(Y);% distance matrix
...
%%
if nargin<4 || isempty(treshold)
    treshold=std(x);
end
%%
[m,n] = size(DM);
[i,j] = find(DM<=treshold);
recurrpt=[i j];
%% RQA
...
if nargin<5 || isempty(LineMinLength)
    LineMinLength = 2;
end
...
if isempty(recurrpt)
    ...
    RR=100*length(recurrpt)/(W*(W-1)/2); % REC or RR
    ...
    %Entropy Calculation
    diag=diag(find(diag>LineMinLength));
    vect = diag(:);
    ...
    vert=vert(find(vert>LineMinLength));
    ...
    if isempty(RR)||isnan(RR);RR = 0; end
    DIV=1/LMAX;
